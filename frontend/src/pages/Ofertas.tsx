...
export default function Ofertas() {
  ...
  const discounted = records.filter((r) => r.compareAtPrice);
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-16">
      ...
      <p className="mt-3 max-w-md text-sm text-[var(--grey)]">
        Ediciones con descuento mientras duren las unidades en stock.
      </p>

      <div className="mt-10 grid grid-cols-2 gap-x-6 gap-y-12 md:grid-cols-4">
        {discounted.map((record) => {
          const discount = Math.round(
            (1 - record.price / record.compareAtPrice!) * 100
          );
          return (
