...
export default function Electronic() {
  usePageTitle("Electronica");
  const { addToCart } = useCart();
  const electronicRecords = records.filter((r) => r.genre === "Electrónica");
  return (
    <section className="mx-auto max-w-6xl px-6 py-16">
      ...
      <p className="mt-3 max-w-md text-sm text-[var(--grey)]">
        Sonidos sintéticos y ritmos bailables, directo al vinilo.
      </p>

      <div className="mt-10 grid grid-cols-2 gap-x-6 gap-y-12 md:grid-cols-4">
        {electronicRecords.map((record) => {
          const discount =
